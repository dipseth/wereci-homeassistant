...
# No shared list for this account: the answer changes only when they (or an
# assistant they asked) share one, so a 30 s poll is only load and trace
# noise. Look again every 5 min; the short interval comes back the moment a
# list appears — and the refresh button / an Assist tool call check at once.
LIST_ABSENT_POLL_INTERVAL = timedelta(minutes=5)
